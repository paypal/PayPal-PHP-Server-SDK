...
```php
use PaypalServerSdkLib\Models\Builders\NetworkTransactionReferenceEntityBuilder;
use PaypalServerSdkLib\Models\CardBrand;

$networkTransactionReferenceEntity = NetworkTransactionReferenceEntityBuilder::init(
    'id4'
)
    ->date('date0')
    ->network(CardBrand::CB_NATIONALE)
    ->time('time4')
    ->build();
```
